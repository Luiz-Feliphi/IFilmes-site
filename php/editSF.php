...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        // Exibe os dados do filme em um formulário para edição
        while ($info_filme = mysqli_fetch_assoc($result)) {
            $nome = $info_filme['nome'];
            $descricao = $info_filme['descricao'];
            $posto_por = $info_filme['posto_por'];
            $link_imagem = $info_filme['imagem_link'];
        }
    } else {
        // Se não encontrar o filme com o ID fornecido, redireciona de volta para a página principal
        echo '<script>alert("Erro ao editar filme (ID não encontrado). Por favor, tente novamente.");</script>';
        header('Location: addF.php');
    }
} else {
    // Se não houver ID de filme na URL, redireciona de volta para a página principal
...
<body class="bg-secondary-subtle">
    ...
    <span class="d-flex justify-content-center">
        <p class="text-center fw-bold fs-2 bg-primary p-2 mt-2 rounded w-50 text-white">Editor de Filme</p>
    </span>

    <div class="d-flex justify-content-start">
        <a id="toggleButton" href="addF.php" class="ms-5 d-flex btn btn-primary icon-link icon-link-hover"
    ...
    <div class="d-flex justify-content-center">
        <img id="previewImage" src="<?php echo htmlspecialchars($link_imagem); ?>" width="286px" height="286px"
            class="rounded mx-auto d-block" alt="Preview">
    </div>
    <!--Ilustração Previa da Imagem Inserida-->
    <div class="d-flex flex-wrap justify-content-center mt-5">
        <!-- Aqui você pode adicionar o formulário para editar os dados do filme -->
        <form class="row g-3" method="POST" action="atualizar_filmeSF.php">
            <!-- Adicione este campo oculto dentro do formulário -->
            ...
            <!--Link da Imagem-->
            <div class="col-md-6">
                <label for="imagem" class="form-label">Link da Imagem do Filme</label>
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control" id="imagem" name="imagem"
                        ...
                        aria-describedby="basic-addon1" value="<?php echo htmlspecialchars($link_imagem); ?>">
                </div>
            </div>
            <!--Link da Imagem-->
            <!--Descrição-->
            <div class="col-12">
                ...
                <div class="input-group">
                    ...
                    <textarea class="form-control" id="descricao" name="descricao"
                        aria-label="Descrição"><?php echo htmlspecialchars($descricao); ?></textarea>
                </div>
            </div>
            <!--Descrição-->
            <!--Sugerido Por-->
            <div class="col-md-6">
                <label for="sugerido_por" class="form-label">Sugerido Por</label>
                <div class="input-group mb-3">
                    <span class="input-group-text" id="basic-addon1"><i class="bi bi-person-fill"></i></span>
                    <input type="text" class="form-control" id="sugerido_por" name="sugerido_por"
                        placeholder="Sugerido Por" aria-label="Sugerido Por" aria-describedby="basic-addon1"
                        value="<?php echo htmlspecialchars($posto_por); ?>">
                </div>
            </div>
            <!--Sugerido Por-->
            <!--Botão de Enviar Edição-->
            <div class="col-12">
                <button type="submit" class="btn btn-primary w-100"><svg xmlns="http://www.w3.org/2000/svg" width="16"
                        height="16" fill="currentColor" class="bi bi-pencil-square me-2" viewBox="0 0 16 16">
                        <path
